...
namespace HowToShowFilteredListViewFromNavigationControl.Module {
	public partial class MyShowNavigationItemController : ShowNavigationItemController {
		public override void UpdateModel(Dictionary dictionary) {
			...
			DictionaryNode tasksNode = dictionary.RootNode.FindChildNode("BOModel").GetChildNode("Class", "Name", typeof(Task).FullName);
			string viewId = tasksNode.FindAttribute("DefaultListView").Value;
			GenerateNavigationItemNode(dictionary, new ViewShortcut(viewId, null), "Default", "TodayTasks", "Today's Tasks");
		}
		protected override void ShowNavigationItem(SingleChoiceActionExecuteEventArgs args) {
			...
			if((args.SelectedChoiceActionItem != null) && args.SelectedChoiceActionItem.Enabled) {
				if(args.SelectedChoiceActionItem.Caption == "Today's Tasks") {
