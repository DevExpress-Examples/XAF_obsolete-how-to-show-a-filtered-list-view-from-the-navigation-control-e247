...
Namespace HowToShowFilteredListViewFromNavigationControl.Module
	...
	Public Class MyNavigationItemsUpdater
		...
		Public Overrides Sub UpdateNode(ByVal node As DevExpress.ExpressApp.Model.Core.ModelNode)
            ...
            Dim taskNode As IModelClass = CType(items.Application.BOModel, IModelList(Of IModelClass))(GetType(Task).FullName)
			Dim view As IModelView = taskNode.DefaultListView
			Dim newItem As IModelNavigationItem = TryCast(items.GetNode("Default"), IModelNavigationItem).AddNode(Of IModelNavigationItem)()
			newItem.View = view
